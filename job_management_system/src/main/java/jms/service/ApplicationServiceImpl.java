...
public class ApplicationServiceImpl implements ApplicationService {
    ...
    public Application updateApplicationStatus(Long id, ApplicationStatus status) {
        ...
        if (currentStatus == status) {
            log.info("Application status is already: {}", status);
            return application; // Không cần cập nhật nếu trạng thái không thay đổi
        }
        ...
        // FORCE FLUSH ĐỂ ĐẢM BẢO DỮ LIỆU ĐƯỢC LƯU NGAY LẬP TỨC
        applicationRepository.flush();

        log.info("Successfully updated application status from {} to {}", currentStatus, status);
